package com.example.investidorapptarefa.viewmodel
...
import com.example.investidorapptarefa.MainActivity
import com.example.investidorapptarefa.R
...
class InvestimentosViewModel(application: Application) : AndroidViewModel(application) {
  ...
  private fun monitorarAlteracoes() {
    database.addChildEventListener(object : ChildEventListener {
      ...
      override fun onCancelled(error: DatabaseError) {
        Log.d("FirebaseError", "Erro ao monitorar alterações: ${error.message}")
      }
    })
  }
  ...
  fun adicionarInvestimento(investimento: Investimento) {
    val novoInvestimentoRef = database.push()
    novoInvestimentoRef.setValue(investimento)
      .addOnSuccessListener {
        Log.d("Firebase", "Investimento adicionado com sucesso!")
      }
      .addOnFailureListener { e ->
        Log.e("FirebaseError", "Erro ao adicionar investimento: ${e.message}")
      }
  }
}